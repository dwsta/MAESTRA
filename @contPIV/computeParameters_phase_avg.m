...
function computeParameters_phase_avg(cfg_data,pivdir,alias,frames,raw_signal,output_file_connection)
try
...
param_out = [repmat({'NaN'},[1,13]),repmat({'{NaN}'},[1,6])];
param_out{1} = alias;
param_out{14} = arr2str_rik(raw_signal);
param_out{15} = arr2str_rik(raw_time);
param_out{16} = arr2str_rik(frames);
...
print(figHandles(1),'-dpng','-r300',fullfile(pivdir,'div_raw.png'))
...
param_out{2} = num2str(t_peak);
param_out{6} = num2str(d_peak);
param_out{7} = num2str(d_valley);
param_out{8} = num2str(d_high);
param_out{9} = num2str(d_low);
param_out{17} = arr2str_rik(phase_time);
param_out{18} = arr2str_rik(div);
param_out{20} = arr2str_rik(t);
