...
function [tauxz,tauyz,tauzz,u,v,w] = forces3D(alpha,beta,h,h0,sigma,uph0,vph0,wph0);
...
elseif k==0;
   ...
   v = vph0;
   w = wph0;
   tauxz = uph0/(1+sigma)/h0/2;
   tauyz = vph0/(1+sigma)/h0/2;
   tauzz = wph0*(1-sigma)/(1+sigma)/(1-2*sigma)/h0;
else
   %
